...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "MakeJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
conn.logger.warn(chalk.bold.redBright(lenguajeGB['smsConexiondescon'](reason, connection)));
await global.reloadHandler(true).catch(console.error);
}}}
...
let prekey = []
let directorio = readdirSync("./MakeBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') || file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./MakeBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./MakeJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./MakeJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./MakeJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') || fileInDir.startsWith('app-') || fileInDir.startsWith('session-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./MakeJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./MakeBotSession/', './MakeJadiBot/']
